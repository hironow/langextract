...
@pytest.mark.parametrize(
    "chunks,expected_deltas,max_buf",
    [
        # Given 'ABC' then 'DEF' then flush => When threshold hit and flush => Then deltas [A,D], then [G]
        (["ABC", "DEF", "GHI"], [["A", "D"], ["G"]], 10),
        # Given 'A' then 'A' again with very small buffer => first push triggers => Then flush returns no new items
        (["A", "A"], [["A"], []], 2),
    ],
)
def test_stream_extractor_given_chunks_when_pushed_then_yields_incremental_deltas(
    monkeypatch: pytest.MonkeyPatch, chunks, expected_deltas, max_buf
):
  ...
  proc = StreamExtractor(model_id="gemini-2.5-flash", max_char_buffer=max_buf)
  ...
  flushed = proc.flush()
  # always record flush result (may be empty) to capture "no new items"
